...
function Harden {
    ...
    -LockoutObservationWindow 00:05:00 -ComplexityEnabled $False -ReversibleEncryptionEnabled $False -LockoutThreshold 5 -MinPasswordLength 15
    ...
    Invoke-Command -SessionName "switch" -ScriptBlock {
        ...
        foreach ($user in $users) {
            if ($user.name -eq 'sys32admin' -or $user.name -eq 'blackteam') {
                continue;
            } else {
